...
const AEExport: React.FC = () => {
  return (
    <div className="faq-content">
      ...
      >
        <p>
          Настройка экспорта композиции из{" "}
          <mark className="app">Adobe After Effects</mark> - ключевой этап, от которого
          зависит, как будет выглядеть ваша анимация, моушн-графика или видео на экране
          зрителя. Грамотно подобранные параметры позволят вам достичь баланса между
          качеством изображения, размером файла и совместимостью с различными устройствами
          и платформами. Единого идеального пресета для экспорта не существует - настройки
          всегда зависят от вашей конкретной задачи, содержимого проекта и требований
          площадки, где будет опубликован финальный ролик.
        </p>
        <Divider>Что вообще влияет на качество после экспорта?</Divider>
        <ul>
          <li>
            На качество изображения композиций после экспорта обычно влияет выбор кодека и
            битрейт.
          </li>
          <li>
            Динамичные сцены со сложными переходами, анимацией с множеством движущихся
            элементов или частиц требуют значительно большего значения битрейта для
            сохранения всех деталей. При низком битрейте на таких сценах могут появляться
            артефакты от сжатия: артефакты в виде квадратов, размытие и потеря чёткости. В
            условиях недостаточного битрейта - чем меньше движения объектов в кадре, тем
            больше битрейта кодек может отдать на детали других движущихся объектов.
            <AdditionWarning>
              Если же файл уже был пережат, допустим, до <mark>1 Мбит/с</mark> - вернуть
              исходное качество изображения, просто повысив битрейт при повторной
              конвертации, не получится. Если установить значение битрейта значительно
              больше, чем у оригинала - вы только лишний раз раздуете размер файла после
              экспорта.
            </AdditionWarning>
            <YouTubeVideo
        ...
        <Divider>
          Как экспортировать видео с максимальным качеством и вменяемым весом?
        </Divider>
        <p>
          Если вы хотите экспортировать композицию с постоянным качеством - используйте
          вариант кодирования <mark>CQ</mark>. Из{" "}
          <mark className="app">Adobe After Effects</mark> экспортировать композицию с
          квантованием для постоянного качества вы можете через{" "}
          <mark className="plugin">Voukoder</mark>.
        </p>
        {/* TODO:  рассказать про квантование / квантирование */}
        <Divider>Как экспортировать композицию без сильных искажений цветов?</Divider>
        <p>
          Кодеки <mark className="video">H.264</mark> и{" "}
          <mark className="video">H.265</mark> любят сжимать цвета.
          {/* TODO: */}
        </p>
        <Divider>Что по настройкам для видеосервисов?</Divider>
        <p>
          Одно дело вывести композицию из <mark className="app">Adobe After Effects</mark>{" "}
          и смотреть в своём видеопроигрываетеле, а другое - публиковать его на
          какую-нибудь платформу, например <mark className="app">YouTube</mark> или{" "}
          <mark className="app">TikTok</mark>. Большинство видеохостингов автоматически
          конвертируют загружаемое видео, используя собственные алгоритмы сжатия, которые
          оптимизированы под их инфраструктуру.
        </p>
        <p>
          Публикация видео с избыточно высокими параметрами экспорта (разрешением,
          частотой кадров и битрейтом) может быть подвергнуто агрессивному сжатию со
          стороны кодировщика платформы. Это может привести к заметному ухудшению качества
          изображения по сравнению с оригинальным файлом. Например, видео, загружаемое в{" "}
          <mark className="app">TikTok</mark> с исходными параметрами{" "}
          <mark>4K, 120 к/с и 200 Мбит/с</mark> будет автоматически преобразовано до
          разрешения <mark>1080p</mark>, частоты кадров <mark>60 к/с</mark> и битрейта
          около <mark>5-10 Мбит/с</mark>.
        </p>
        <Divider>Какое количество кадров в секунду нужно ставить при экспорте?</Divider>
        <p>
          Если частота кадров исходного материала и проекта не совпадают - это может
          привести к нежелательным артефактам при воспроизведении. Например, если вы
          импортировали видео с частотой <mark>25 кадров в секунду</mark>, но в настройках
          композиции вы указали значение <mark>30 кадров в секунду</mark> - некоторые
          кадры будут повторяться, что может привести к дёрганному движению на видео.
          Чтобы избежать подобных проблем - убедитесь в том, что частота кадров вашего
          проекта соответствует частоте кадров исходного материала. Это можно посмотреть в
          окне <mark className="ui">Project</mark> с помощью столбца{" "}
          <mark className="ui">Frame Rate</mark>.
        </p>
        <AdditionInfo>
          Если вы импортировали видео с плавающим значением частоты кадров, например
          снятые на <mark>iPhone</mark> - их стоит конвертировать в постоянные{" "}
          <mark>30</mark> или <mark>60</mark> кадров в секунду с помощью конвертера,
          например <mark className="app">Shutter Encoder</mark>. Конвертированные видео
          будет проще редактировать и ротоскопировать.
        </AdditionInfo>
        <YouTubeVideo
      ...
      >
        <p>
          ...
          встречаемся каждый день - в социальных сетях, видеохостингах, на сайтах и даже в
